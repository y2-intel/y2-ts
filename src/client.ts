...
export interface ClientOptions {
  /**
   * API keys in format `y2_{64_hex_chars}`.
   ...
   *
   * Defaults to process.env['Y2_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['Y2_LOG'] or 'warn' if it isn't set.
   */
...
export class Y2 {
   ...
   */
  constructor({
    baseURL = readEnv('Y2_BASE_URL'),
    apiKey = readEnv('Y2_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.Y2Error(
        "The Y2_API_KEY environment variable is missing or empty; either provide it, or instantiate the Y2 client with an apiKey option, like new Y2({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Y2.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('Y2_LOG'), "process.env['Y2_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('Y2_CUSTOM_HEADERS');
    if (customHeadersEnv) {
